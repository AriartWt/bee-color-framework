...
final class KVSUserGroup implements IKVSUserGroup {
	...
	public function __construct(string $name, array $users) {
		...
		foreach ($users as $offset=>$user){
			if($user instanceof IKVSUser) $this->_users[$user->getName()]=$user;
			else throw new \InvalidArgumentException(
				"Invalid argument at offset $offset : only object that implements "
				.IKVSUser::class." are allowed !"
			);
		}
	}
	...
	public function getUser(string $name): IKVSUser {
		if($this->hasUser($name)) return $this->_users[$name];
		else throw new UserNotFound("Unknwown user $name");
	}
