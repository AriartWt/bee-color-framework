...
abstract class PDOBasedDBAccess implements IDBAccess
{
     ...
     */
    public function __construct(string $db,string $host, string $dbName, string $login, string $password)
    {
        $this->_db = $db;
        $this->_host = $host;
        $this->_login = $login;
    ...
    protected function getPDOType($data){
        if(is_bool($data)){
            ...
            throw new InvalidArgumentException("No corresponding PDO::PARAM_* constant value for given value ! (".gettype($data).")");
        }
    }
