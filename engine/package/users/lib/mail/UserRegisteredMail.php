 ...
 */
final class UserRegisteredMail extends AbstractUserMail{
	/**
	...
	public function __construct(
		...
		parent::__construct(
			...
			new EmailSubject($translator->get(
				"server/engine/package/users/mail/subject/CONFIRM_REGISTRATION"
			)),
	...
	protected function createBody(User $user, UserConfirmationCode $code): IMailBody {
		$addr = $this->getDomain()."/users/confirmUserRegistration?id=".$user->getId()."&code=$code";
		$key="server/engine/package/users/mail";
		return new MailBody(
			$this->_translator->getAndReplace("$key/INTRO",$user->getLogin())
			.$this->_translator->getAndReplace("$key/explain/CONFIRM_REGISTRATION",$addr)
