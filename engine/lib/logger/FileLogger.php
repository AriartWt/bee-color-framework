...
class FileLogger implements ILogger {
	 ...
	 * @param string|resource ...$files Fichiers de logs utilisables
	 * @throws PermissionDenied
	 */
	public function __construct(ILogFormater $formater,... $files) {
		$this->_files = [];
		foreach($files as $k=>$f){
			$index = (string)pow(2,$k);
			if(is_resource($f)) $this->_handles[$index] = $f;
			else{
				if(!file_exists($f)) touch($f);
				if(!is_writable($f)) throw new PermissionDenied("no write access to $f");
				$this->_handles[$index] = fopen($f,"a");
			}
			$this->_files[$index]=$f;
		}
		$this->_merges = [];
		...
		foreach($merges as $m){
			if(!isset($exclusions["$m"]) && $this->isEnabled($m)){
				if(!$this->applyRedirections($m,$message,$exclusions)) {
					$this->write($m,$message,$exclusions,$type);
				}
			}
		}
	}
	 ...
	 * @param int|null $asType  Type à utiliser pour le formater
	 */
	private function write(int $type,string $message, array &$exclusions, ?int $asType=null):void{
		if($this->isEnabled($type) && !isset($exclusions["$type"])){
			$f=$this->_handles["$type"];
			flock($f,LOCK_EX);
			fwrite($f,$this->_formater->format($message,$asType??$type));
			flock($f,LOCK_UN);
	 ...
	 * @return FileLogger
	 */
	public final function autoConfFileByLevel(int $level, int $to, bool $merge = true):ILogger{
	 ...
	 * @param bool $enable Si true, active les fichiers désignés par level et désactive les autres.
	 *                     Si false, désactive les fichiers désingés par level et active les autres
	 * @return FileLogger
	 */
	public final function autoConfByLevel(int $level, bool $enable = true):ILogger{
		$bytes = array_reverse(str_split(decbin($level)));
		$match = [];
		$dismatch = [];
		foreach($bytes as $k=>$b){
			if($b==='1') $match[]=pow(2,$k);
			else $dismatch[]=pow(2,$k);
		}
		$this->disable(...array_keys($this->_files));
		if($enable) $this->enable(...$match);
		else $this->enable(...$dismatch);
		return $this;
	}
}